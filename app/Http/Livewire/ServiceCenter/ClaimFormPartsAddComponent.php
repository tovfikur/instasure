...
class ClaimFormPartsAddComponent extends Component
{
    use WithFileUploads;
    public $insurance;
    public $details;
    ...
    public function mount($insurance, $details, $deviceInfo)
    {
        ...
        if (count($details)) {
            ...
            if ($details[0]->parts_type == 'Screen Protection') {
                ...
                $this->claim_amount = $this->insurance->claimable_amount;
            }
        }
    }
    ...
    public function claimSubmit()
    {
        ...
        // dd($deviceInsurance->claimable_amount);
        ...
        if ($deviceInsurance->customer_will_pay_charge == 1) {
            if ($this->totalAmount > $deviceInsurance->claimable_amount && $deviceInsurance->claimable_amount != 0) {

                $deviceClaim->amount_will_pay_ins_provider = $deviceInsurance->claimable_amount - userWillPay($deviceInsurance->claimable_amount);
                $extra_amount_for_user = $this->totalAmount -  $deviceInsurance->claimable_amount;
                $deviceClaim->user_will_pay = $extra_amount_for_user  + userWillPay($deviceInsurance->claimable_amount);
            ...
            if ($this->totalAmount <= $deviceInsurance->claimable_amount && $deviceInsurance->claimable_amount != 0) {
                ...
                $deviceInsurance->claimable_amount =  $deviceInsurance->claimable_amount - $this->totalAmount;
            }
        } else {
        ...
        $deviceClaim->device_value = $device_info->device_price;
        ...
        $smsResponse = SmsService::send_sms($customerPhone, env('SMS_API_CLAIM_ACCEPTED'));;
