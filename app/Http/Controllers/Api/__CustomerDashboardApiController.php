...
class CustomerDashboardApiController extends ApiController
{
    ...
    {
        try {
            $deviceClaimRequest = DeviceClaimRequest::with(['service_center', 'deviceInsurance'])->where('user_id', Auth::id())->paginate(1);
    ...
    public function travel_insurance_order_create(TravelInsuranceOrderCreateRequest $request)
    {
    ...
    public function mobile_diagnosis_report_store(Request $request)
    {
        ...
        try {
            ...
            if (!empty($request->hasFile('invoice_image'))) {
                ...
                if ($mobileDiagnosis->invoice_image) {
                    $image_path = public_path('/') . $mobileDiagnosis->invoice_image;
                    $this->delete_file($image_path);
                }

                ### Upload process
                $mobileDiagnosis->invoice_image = $request->invoice_image->store('uploads/mobile_diagnosis/invoice_images');
            }
            ...
            if (!empty($request->hasFile('device_images'))) {
                ...
                foreach ($request->device_images as $image) {
                    $path = $image->store('uploads/mobile_diagnosis/device_images');
                    array_push($device_images, $path);
                }
            ...
            if (!empty($request->hasFile('imei_image'))) {
                ### Delete existing image
                if ($mobileDiagnosis->imei_image) {
                    $image_path = public_path('/') . $mobileDiagnosis->imei_image;
                    $this->delete_file($image_path);
                }

                ### Upload process
                $mobileDiagnosis->imei_image = $request->imei_image->store('uploads/mobile_diagnosis/imei_images');
            }
