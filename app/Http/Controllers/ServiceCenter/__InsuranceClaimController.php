...
class InsuranceClaimController extends Controller
{
    ...
    public function StatusChange(Request $request)
    {
        ...
        try {
            ...
            $claim->save();
            ## Send sms on mobile
            $message = env('SMS_API_FAILED');
            $status = strtolower($request->status);
            if ($status == 'on processing') {
                $message =  env('SMS_API_CLAIM_PROCESSING');
            } elseif ($status == 'on delivered') {
                $message =  env('SMS_API_CLAIM_READY_TO_DELIVER');
            } elseif ($status == 'delivered') {
                $message =  env('SMS_API_CLAIM_DELIVERED');
            } elseif ($status == 'cancel') {
                $message =  env('SMS_API_CLAIM_CANCELED');
    ...
    public function requestToPaymentFromParent(Request $request)
    {
        ...
        if ($request->select_action_type != null && isset($request->claim_ids)) {
            ...
            $claimPaymentRequest->requestId = 'CRTP-' . mt_rand(11111111, 99999999);
