                ...
                <div class="card card-info card-outline">
                    ...
                    <div class="card-body table-responsive">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @foreach ($parentDealers as $key => $parentDealer)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>
                                            <img src="{{ asset('uploads/dealer-logo/photo/' . $parentDealer->logo) }}"
                                                width="30" height="20" alt="dealar-log">
                                        </td>

                                        <td>{{ $parentDealer->com_org_inst_name }}</td>

                                        <td>{{ $parentDealer->contact_person_name }}</td>
                                        <td>{{ $parentDealer->contact_person_phone }}</td>

                                        <td>{{ $parentDealer->dealer_type }}</td>
                                        <td>
                                            <span
                                                class="badge badge-{{ $parentDealer->imei_check ? 'success' : 'secondary' }}">
                                                @if ($parentDealer->imei_check)
                                                    Yes
                                                @else
                                                    No
                                                @endif
                                        ...
                                        <td>
                                            <span
                                                class="badge badge-{{ $parentDealer->active ? 'success' : 'secondary' }}">
                                                @if ($parentDealer->active)
                                                    Active
                                                @else
                                                    Inactive
                                                @endif
                                            ...
                                            <div class="dropleft">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    <a class="bg-dark dropdown-item"
                                                        href="{{ route('admin.parent-dealers.show', $parentDealer->id) }}">
                                                        <i class="fa fa-eye text-success"></i> Details
                                                    </a>
                                                    <a class="bg-dark dropdown-item"
                                                        href="{{ route('admin.parent-dealers.edit', $parentDealer->id) }}">
                                                        <i class="fa fa-edit text-info"></i> Edit
                                                    </a>

                                                    <a class="bg-dark dropdown-item btn_brand_mapping"
                                                        href="{{ route('admin.parent_dealers_brand_mapping', $parentDealer->id) }}">
