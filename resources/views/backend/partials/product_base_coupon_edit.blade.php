...
<div class="panel-heading">
    <h3 class="panel-title">Update Your Product Base Coupon</h3>
</div>
        ...
        <div class="form-group" id="subcategory">
            ...
            <div class="col-lg-9">
                <select class="form-control subcategory_id demo-select2" name="subcategory_ids[]" required>
                    @foreach(\App\Model\Subcategory::where('category_id', $details->category_id)->get() as $key => $subcategory)
                        <option value="{{$subcategory->id}}" {{$subcategory->id == $details->subcategory_id ? 'selected' : ''}}>{{$subcategory->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-lg-9">
                <select name="product_ids[]" class="form-control product_id demo-select2" required>
                    @foreach(\App\Model\Product::where('subcategory_id', $details->subcategory_id)->get() as $key => $product)
